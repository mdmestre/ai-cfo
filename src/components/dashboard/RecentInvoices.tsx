    ...
    <div className="metric-card animate-slide-up">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <p className="text-[13px] font-medium text-muted-foreground">Faturas recentes</p>
        </div>
        <button
      ...
      {recent.length > 0 ? (
            ...
            >
              <div className="flex items-center gap-3">
                <div
                  className={`rounded-md p-1.5 ${
                    inv.direction === "receivable" ? "bg-success/8" : "bg-primary/8"
                  }`}
                >
                ...
                <div>
                  <p className="text-[13px] font-medium text-foreground">{inv.invoice_number}</p>
                  <p className="text-[11px] text-muted-foreground">
                    {inv.customer?.name || inv.vendor?.name || "-"} ·{" "}
                    {inv.due_date ? `Vence ${format(parseISO(inv.due_date), "dd/MM", { locale: ptBR })}` : "Sem vencimento"}
                  </p>
                </div>
              </div>
              ...
              <div className="text-right">
                <p className="text-[13px] font-semibold text-foreground">
                  {formatBRLNoCents(Number(inv.total_amount))}
                </p>
                <span
        ...
        <p className="text-[13px] text-muted-foreground py-8 text-center">
          Nenhuma fatura ainda. Crie a primeira para comecar.
        </p>
      )}
    </div>
  );
