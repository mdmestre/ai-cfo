...
const TRIGGER_TYPES = [
  ...
  { value: "balance_low", label: "Caixa abaixo do limite" },
  { value: "expense_above_limit", label: "Despesa acima do limite" },
  { value: "payment_received", label: "Recebimento confirmado" },
  { value: "daily_summary", label: "Resumo diario" },
];
...
const ACTION_TYPES = [
  ...
  { value: "auto_categorize", label: "Auto-categorizar" },
  { value: "flag_review", label: "Marcar para revisao" },
];
    ...
    <AppLayout>
      <div className="max-w-[1120px] space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-[20px] font-bold text-foreground tracking-tight">Automacao</h1>
            <p className="mt-0.5 text-[13px] text-muted-foreground">
              Crie regras para automatizar alertas, categorizacao e rotinas do financeiro.
            </p>
          </div>
          <button
        ...
        {showForm && (
          <div className="metric-card animate-slide-up space-y-4">
            <p className="text-[14px] font-semibold text-foreground">Criar regra de automacao</p>
            ...
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
              <div>
                <label className="text-[11px] font-medium text-muted-foreground mb-1 block">Quando (gatilho)</label>
                <select
              ...
              <div>
                <label className="text-[11px] font-medium text-muted-foreground mb-1 block">Entao (acao)</label>
                <select
              ...
              <div>
                <label className="text-[11px] font-medium text-muted-foreground mb-1 block">Limite (opcional)</label>
                <input
                  type="number"
            ...
            <div className="flex gap-2">
              ...
              >
                {createRule.isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : "Criar regra"}
              </button>
        ...
        {isLoading ? (
                  ...
                  <div className="flex items-center gap-3 flex-1 min-w-0">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        ...
                        {!rule.is_active && <span className="rounded-full bg-muted px-2 py-0.5 text-[10px] text-muted-foreground">Pausada</span>}
                      </div>
                      <p className="text-[12px] text-muted-foreground mt-0.5">
                        Quando <span className="font-medium text-foreground">{TRIGGER_TYPES.find(t => t.value === rule.trigger_type)?.label || rule.trigger_type}</span>
                        {" -> "}
                      ...
                      <div className="flex items-center gap-3 mt-1 text-[11px] text-muted-foreground">
                        <span>Disparos: {rule.trigger_count}x</span>
                        {rule.last_triggered_at && (
                          <span className="flex items-center gap-1">
                            <Clock className="h-3 w-3" />
                            Ultimo: {format(new Date(rule.last_triggered_at), "dd/MM HH:mm")}
                          </span>
                        )}
                      </div>
                    </div>
          ...
          <div className="metric-card flex flex-col items-center justify-center py-16">
            ...
            <p className="text-[14px] font-medium text-foreground">Nenhuma regra ainda</p>
            <p className="mt-1 text-[13px] text-muted-foreground">Crie sua primeira regra para automatizar o financeiro.</p>
          </div>
        )}
        ...
        {logs.length > 0 && (
          <div className="metric-card animate-slide-up">
            <p className="section-label mb-3">Log de execucao</p>
              ...
              {logs.slice(0, 10).map((log: any) => (
                <div key={log.id} className="flex items-center justify-between py-2 border-b border-border/30 last:border-0 text-[12px]">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-foreground font-medium">{log.automation_rules?.name || "Regra"}</span>
                  </div>
                  <span className="text-muted-foreground">{format(new Date(log.executed_at), "dd/MM HH:mm")}</span>
                </div>
              ))}
            </div>
          </div>
