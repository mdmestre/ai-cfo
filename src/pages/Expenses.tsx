    ...
    <AppLayout>
      <div className="max-w-[1200px] space-y-6 animate-fade-in">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-xl font-semibold text-foreground tracking-tight">Expense Management</h1>
            <p className="mt-0.5 text-[13px] text-muted-foreground">Track, approve, and manage company expenses</p>
          </div>
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
          ...
          <div className="metric-card">
            <div className="flex items-center gap-2 mb-1">
              ...
              <p className="text-[13px] font-medium text-muted-foreground">Pending Approval</p>
            </div>
          ...
          <div className="metric-card bg-primary text-primary-foreground border-0">
            <div className="flex items-center gap-2 mb-1">
              ...
              <p className="text-[13px] font-medium text-primary-foreground/70">Approved</p>
            </div>
        ...
        {showCatForm && (
          <form onSubmit={handleCreateCategory} className="metric-card space-y-3 animate-slide-up">
            <p className="text-[14px] font-bold text-foreground">New Category</p>
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-4">
              <input type="text" placeholder="Name" value={catForm.name} onChange={(e) => setCatForm({ ...catForm, name: e.target.value })} required className="rounded-md border border-border bg-card px-3 py-2 text-[13px] outline-none" />
              <input type="text" placeholder="Code (e.g. TRAVEL)" value={catForm.code} onChange={(e) => setCatForm({ ...catForm, code: e.target.value })} required className="rounded-md border border-border bg-card px-3 py-2 text-[13px] outline-none" />
              <input type="number" step="0.01" placeholder="Budget limit (optional)" value={catForm.budget_limit} onChange={(e) => setCatForm({ ...catForm, budget_limit: e.target.value })} className="rounded-md border border-border bg-card px-3 py-2 text-[13px] outline-none" />
              <button type="submit" disabled={createCategory.isPending} className="rounded-md bg-foreground px-3 py-2 text-[13px] font-medium text-background hover:opacity-90 disabled:opacity-50">Create</button>
            </div>
          </form>
        )}
        ...
        {showForm && (
          <form onSubmit={handleCreate} className="metric-card space-y-3 animate-slide-up">
            <p className="text-[14px] font-bold text-foreground">Submit Expense</p>
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
              ...
              <input type="text" placeholder="Description" value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} required className="rounded-md border border-border bg-card px-3 py-2 text-[13px] outline-none" />
              <input type="text" placeholder="Merchant" value={form.merchant} onChange={(e) => setForm({ ...form, merchant: e.target.value })} className="rounded-md border border-border bg-card px-3 py-2 text-[13px] outline-none" />
            ...
            <button type="submit" disabled={createExpense.isPending} className="rounded-md bg-primary px-4 py-2 text-[13px] font-bold text-primary-foreground hover:opacity-90 disabled:opacity-50">
              {createExpense.isPending ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : "Submit Expense"}
            </button>
          </form>
        )}
        ...
        <div className="flex gap-1">
          {(["all", "pending", "approved", "rejected"] as const).map((tab) => (
            <button key={tab} onClick={() => setActiveTab(tab)} className={`rounded-md px-3 py-1.5 text-[13px] font-medium capitalize transition-colors ${activeTab === tab ? "bg-foreground text-background" : "border border-border bg-card text-foreground hover:bg-secondary"}`}>
              {tab} {tab !== "all" && `(${expenses.filter((e: any) => e.status === tab).length})`}
            </button>
          ))}
        </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-border">
                {["Status", "Date", "Description", "Merchant", "Category", "Amount", "Actions"].map((h) => (
            ...
            <tbody>
              {filtered.length === 0 ? (
                <tr><td colSpan={7} className="px-4 py-12 text-center text-[13px] text-muted-foreground">No expenses found.</td></tr>
              ) : (
                filtered.map((e: any) => (
                  <tr key={e.id} className="border-b border-border last:border-0 hover:bg-secondary/50">
                    ...
                    <td className="px-4 py-3 text-[13px] text-muted-foreground">{format(new Date(e.expense_date), "MMM d")}</td>
                    ...
                    <td className="px-4 py-3">
                      {e.status === "pending" && (
                        <div className="flex gap-1">
                          <button onClick={() => approveExpense.mutate({ id: e.id, approved: true })} className="rounded p-1 text-primary hover:bg-primary/10" title="Approve">
                            <CheckCircle className="h-4 w-4" />
                          </button>
                          <button onClick={() => approveExpense.mutate({ id: e.id, approved: false })} className="rounded p-1 text-destructive hover:bg-destructive/10" title="Reject">
