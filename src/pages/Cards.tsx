...
const Cards = () => {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success("Cartão emitido com sucesso");
      setForm({ holder_name: "", card_type: "virtual", spending_limit: "" });
      setShowForm(false);
    } catch (err: any) {
      toast.error(err.message || "Falha ao emitir cartão");
    }
  };
    ...
    <AppLayout>
      <div className="max-w-[1200px] space-y-6 animate-fade-in">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-xl font-semibold text-foreground tracking-tight">Cartões Corporativos</h1>
            <p className="mt-0.5 text-[13px] text-muted-foreground">Emita cartões virtuais e monitore gastos da equipe</p>
          </div>
          <button onClick={() => setShowForm(!showForm)} className="flex items-center gap-1.5 rounded-md bg-primary px-3 py-1.5 text-[13px] font-medium text-primary-foreground hover:opacity-90">
            <Plus className="h-3.5 w-3.5" /> Emitir Cartão
          </button>
        </div>
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
          <div className="metric-card">
            <div className="flex items-center gap-2 mb-1">
              ...
              <p className="text-[13px] font-medium text-muted-foreground">Cartões Ativos</p>
            </div>
          ...
          <div className="metric-card">
            <div className="flex items-center gap-2 mb-1">
              ...
              <p className="text-[13px] font-medium text-muted-foreground">Limite Total</p>
            </div>
          ...
          <div className="metric-card bg-primary text-primary-foreground border-0">
            <div className="flex items-center gap-2 mb-1">
              ...
              <p className="text-[13px] font-medium text-primary-foreground/70">Gasto no Mês</p>
            </div>
        ...
        {showForm && (
          <form onSubmit={handleCreate} className="metric-card space-y-3 animate-slide-up">
            <p className="text-[14px] font-bold text-foreground">Emitir Novo Cartão</p>
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-4">
              <input type="text" placeholder="Nome do titular" value={form.holder_name} onChange={(e) => setForm({ ...form, holder_name: e.target.value })} required className="rounded-md border border-border bg-card px-3 py-2 text-[13px] outline-none" />
              ...
              <div className="relative">
                ...
                <input type="number" step="0.01" placeholder="Limite de gasto" value={form.spending_limit} onChange={(e) => setForm({ ...form, spending_limit: e.target.value })} required className="w-full rounded-md border border-border bg-card pl-9 pr-3 py-2 text-[13px] outline-none" />
              </div>
              <button type="submit" disabled={createCard.isPending} className="rounded-md bg-foreground px-3 py-2 text-[13px] font-medium text-background hover:opacity-90 disabled:opacity-50">
                {createCard.isPending ? <Loader2 className="h-3.5 w-3.5 animate-spin mx-auto" /> : "Emitir"}
              </button>
            </div>
          </form>
        )}
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {cards.length === 0 ? (
            <div className="col-span-full py-12 flex flex-col items-center justify-center rounded-xl border border-dashed border-border bg-secondary/10">
              ...
              <p className="text-[13px] font-medium text-muted-foreground">Nenhum cartão emitido ainda.</p>
            </div>
          ) : (
            cards.map((card) => {
                ...
                <div key={card.id} className="metric-card group relative overflow-hidden">
                  ...
                  <div className={`rounded-xl p-5 mb-4 ${card.status === "active" ? "bg-gradient-to-br from-primary to-primary/70" : "bg-gradient-to-br from-muted to-muted/70"} text-white`}>
                    <div className="flex items-center justify-between mb-6">
                      <span className="text-[11px] font-bold uppercase tracking-widest opacity-70">{card.card_type === "virtual" ? "Virtual" : "Físico"}</span>
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center justify-between text-[13px]">
                      <span className="text-muted-foreground">Gasto / Limite</span>
                  ...
                  <div className="flex gap-2 mt-4">
                    {card.status === "active" ? (
                      ...
                      <button onClick={() => toggleCardStatus.mutate({ id: card.id, status: "active" })} className="flex items-center gap-1.5 rounded-md border border-border px-2.5 py-1 text-[12px] font-medium text-primary hover:bg-primary/5">
                        <Play className="h-3 w-3" /> Descongelar
                      </button>
                    ) : null}
        ...
        {cardTransactions.length > 0 && (
          <div className="overflow-hidden rounded-lg border border-border bg-card">
            <div className="px-4 py-3 border-b border-border">
              <p className="text-[14px] font-bold text-foreground">Transações Recentes</p>
            </div>
            <table className="w-full">
              <thead>
                <tr className="border-b border-border">
                  {["Data", "Cartão", "Estabelecimento", "Categoria", "Valor", "Status"].map((h) => (
              ...
              <tbody>
                {cardTransactions.map((tx: any) => (
                  <tr key={tx.id} className="border-b border-border last:border-0 hover:bg-secondary/50">
                    <td className="px-4 py-3 text-[13px] text-muted-foreground">{format(new Date(tx.transaction_date), "dd/MM HH:mm", { locale: ptBR })}</td>
                    ...
                    <td className="px-4 py-3"><span className={`text-xxs font-bold uppercase ${tx.status === "completed" ? "text-primary" : "text-muted-foreground"}`}>{tx.status === "completed" ? "Concluído" : tx.status}</span></td>
                  </tr>
                ))}
              </tbody>
            </table>
